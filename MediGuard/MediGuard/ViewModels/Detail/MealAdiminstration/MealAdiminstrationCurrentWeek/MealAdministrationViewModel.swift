...
class MealAdministrationViewModel: ObservableObject {
     ...
     */
    func validateAndAddMeal(userId: String?, mealName: String, description: String, selectedDate: Date, selectedTime: String, photo: UIImage?) {
        ...
        guard let intakeTime = formatter.date(from: selectedTime) else {
            alertMessage = "Ungültige Uhrzeit."
            showAlert = true
            return
        }

        var components = Calendar.current.dateComponents([.year, .month, .day], from: selectedDate)
        let timeComponents = Calendar.current.dateComponents([.hour, .minute], from: intakeTime)
